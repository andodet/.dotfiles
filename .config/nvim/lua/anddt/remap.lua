...
if vim.g.vscode then
  -- vscode.neovim plugin mappings
  -- change the colorscheme for good measure
  vim.cmd('colorscheme default')
  function quickFileOpen() vim.fn.VSCodeNotify("workbench.action.quickOpen") end

  vim.keymap.set("n", "<leader>ff", quickFileOpen, { desc = "Open the quick file explorer", silent = true })

  function termFocus() vim.fn.VSCodeNotify("workbench.action.terminal.toggleTerminal") end

  vim.keymap.set("n", "<leader>t", termFocus, { desc = "Foscus on the terminal pane", silent = true })

  function openFileExplorer() vim.fn.VSCodeNotify("workbench.files.action.focusFilesExplorer") end

  vim.keymap.set("n", "<leader>e", openFileExplorer, { desc = "open VSCode file explorer in sidebar" })

  -- lsp find symbols
  function findSymbolInFile() vim.fn.VSCodeNotify("workbench.action.gotoSymbol") end

  vim.keymap.set("n", "<leader>fs", findSymbolInFile, { desc = "find symbol in file" })
  function findSymbolInWorkspace() vim.fn.VSCodeNotify("workbench.action.showAllSymbols") end

  vim.keymap.set("n", "<leader>fS", findSymbolInWorkspace, { desc = "show all symbols in workspace" })
  function goToDefinition() vim.fn.VSCodeNotify("editor.action.revealDefinition") end

  vim.keymap.set("n", "<leader>gd", goToDefinition, { desc = "go to symbol definition" })
  function openDefinitionToSide() vim.fn.VSCodeNotify("editor.action.revealDefinitionAside") end

  vim.keymap.set("n", "gd", openDefinitionToSide, { desc = "go to symbol definition" })
  function showReferences() vim.fn.VSCodeNotify("editor.action.referenceSearch.trigger") end

  vim.keymap.set("n", "<leader>gr", showReferences, { desc = "show references of symbol under cursor" })
  function showAllReferences() vim.fn.VSCodeNotify("references-view.findReferences") end

  vim.keymap.set("n", "<leader>gR", showAllReferences, { desc = "show all references of symbol under cursor" })
  function goToTypeDefinition() vim.fn.VSCodeNotify("editor.action.goToTypeDefinition") end

  vim.keymap.set("n", "<leader>gt", goToTypeDefinition, { desc = "go to type definition" })

  -- moving around
  --function focusEditorRight() vim.fn.VSCodeNotify("workbench.action.focusRightGroup") end
  --vim.keymap.set("n", "<C-l>", focusEditorRight, { desc = "move to the editor on the right"})
  --function focusEditorLeft() vim.fn.VSCodeNotify("workbench.action.focusLeftGroup") end
  --vim.keymap.set("n", "<C-h>", focusEditorLeft, { desc = "move to the editor on the left"})

  -- git movements
  function showNextChange() vim.fn.VSCodeNotify("editor.action.dirtydiff.next") end

  vim.keymap.set("n", "<leader>gj", showNextChange, { desc = "Show the next change" })
  function showPrevChange() vim.fn.VSCodeNotify("editor.action.dirtydiff.previous") end

  vim.keymap.set("n", "<leader>gk", showPrevChange, { desc = "Show the previous change" })
  function openFileChanges() vim.fn.VSCodeNotify("git.openChange") end

  vim.keymap.set("n", "<leader>gc", openFileChanges, { desc = "Open diff view for the current file" })
  function stageSelectedChange() vim.fn.VSCodeNotify("git.stageSelectedRanges") end

  vim.keymap.set("v", "<leader>gs", stageSelectedChange, { desc = "Stage currently selected range" })
  function unstageSelectedChange() vim.fn.VSCodeNotify("git.unstageSelectedRanges") end

  vim.keymap.set("v", "<leader>gu", unstageSelectedChange, { desc = "Stage currently selected range" })
  vim.cmd("colorscheme default")

  function saveFile() vim.fn.VSCodeNotify("workbench.action.files.save") end

  vim.keymap.set("v", "<leader>w", saveFile, { desc = "save file" })
else
